...
1. **No client-side JavaScript.** Everything renders server-side; charts are inline SVG. Tabs are plain links.
...
- For code changes, verify by running the app and observing behavior — prefer the browser-preview MCP tools over manual checks:
  ...
  - Confirm the rendered HTML has **no `<script>` tags** and no console errors.

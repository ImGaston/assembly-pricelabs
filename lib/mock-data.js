...
/**
 * Generate mock recent reservations matching the shape of lib/reservations.js,
 * so the demo client showcases the Recent Reservations section.
 * @param {Object[]} listings - Array of {listingId, name, city}
 * @param {number} count - How many reservations to generate
 * @returns {Object[]} Reservation rows consumed by lib/render.js
 */
function generateMockReservations(listings, count = 6) {
  if (!listings || listings.length === 0) return [];
  const channels = ['airbnb', 'airbnb', 'vrbo', 'airbnb', 'bcom', 'manual'];
  const now = new Date();

  return Array.from({ length: count }, (_, i) => {
    const listing = listings[i % listings.length];
    const rng = seededRandom(hashString(listing.listingId) + i);

    const bookedAt = new Date(now);
    bookedAt.setDate(bookedAt.getDate() - i * 2 - randomBetween(rng, 0, 1));
    const checkIn = new Date(bookedAt);
    checkIn.setDate(checkIn.getDate() + randomBetween(rng, 10, 90));
    const nights = randomBetween(rng, 2, 7);
    const checkOut = new Date(checkIn);
    checkOut.setDate(checkOut.getDate() + nights);
    const adr = randomBetween(rng, 180, 480);
    ...
    return {
      ...
      revenue: adr * nights,
      adr,
      bookingWindow: Math.round((checkIn - bookedAt) / 86400000),
      currency: 'USD',
    };
  });
}
...
export { generateMockData, generateMockReportData, generateMockReservations, generateMockSeoData };

...
<style>
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .seo-view .row-cta { display:inline-block; font-family:${f.body}; font-weight:700; font-size:9px; text-transform:uppercase; letter-spacing:.5px; color:${c.cedar}; opacity:.6; margin-top:2px; transition:opacity .15s ease, transform .15s ease; }
  .seo-view a.row-link:hover .row-cta { opacity:1; transform:translateX(3px); }
  .seo-view a.row-link .pn { color:${c.cedar}; }
  .seo-view a.row-link:hover .pn { text-decoration:underline; }
  ...
  /* Pricing table — clickable listing rows (drill-down). The affordance must be
     visible at rest, not only on hover: cedar link-colored name + always-on CTA. */
  .ld-row-link { display:block; text-decoration:none; color:inherit; cursor:pointer; }
  .ld-row-link span:first-child { color:${c.cedar}; }
  .ld-row-link:hover span:first-child { text-decoration:underline; }
  .ld-row-link .row-cta { display:inline-block; font-family:${f.body}; font-weight:700; font-size:9px; text-transform:uppercase; letter-spacing:.5px; color:${c.cedar}; opacity:.6; margin-top:2px; transition:opacity .15s ease, transform .15s ease; }
  .ld-row-link:hover .row-cta { opacity:1; transform:translateX(3px); }
...
// --- Recent reservations section (shared by report + legacy layouts) ---
...
// 'YYYY-MM-DD' or ISO timestamp → "Aug 5" (adds year when not the current one).
function formatResDate(value) {
  if (!value) return '—';
...
function renderRecentReservations(reservations) {
  ...
  const rows = reservations
    .map((r) => {
      const channel = CHANNEL_LABELS[r.channel] || (r.channel ? escapeHtml(r.channel) : '—');
      return `
      <tr>
        <td><div style="font-weight:600;">${escapeHtml(r.listingName)}</div></td>
        <td class="mono">${formatResDate(r.bookedAt)}</td>
        <td class="mono">${formatResDate(r.checkIn)} → ${formatResDate(r.checkOut)}</td>
        <td class="mono">${r.nights != null ? r.nights : '—'}</td>
        <td>${channel}</td>
        <td class="mono">${formatCurrency(r.adr)}</td>
        <td class="mono">${r.bookingWindow != null ? Math.round(r.bookingWindow) + 'd' : '—'}</td>
        <td class="mono">${formatCurrency(r.revenue)}</td>
      </tr>`;
    })
    .join('');
  ...
  return `
<div class="table-section">
  <p class="table-section-title">Recent Reservations</p>
  <div class="table-wrap">
    <table>
      <thead>
        <tr>
...
// Single template literal on purpose: SWC's minifier (Next 14.2.35) drops the
// static `?tab=…` chunk when folding `` `…?tab=x` + `&token=${…}` `` inlined
// into another template, breaking the link with a 404. Do not split with `+`.
function pricingBackHref(client) {
  return `/api/dashboard/${encodeURIComponent(client.id)}?tab=pricing&token=${encodeURIComponent(client.token || '')}`;
}
...
// ---- Overview ----
function ldOverview(listing, asOf, reservations) {
  ...
  // Highlights — client-facing card: positive signals only. Internal action
  // items (below-market flags, "check rates" prompts) don't belong in a report
  // the client reads; RevFactor raises those directly, not via the dashboard.
  const flags = [];
  fwd.forEach((m) => {
    if (m.revenueYoy != null && m.revenueYoy > 25 && m.revparIndex != null && m.revparIndex >= 100) {
      flags.push({ lvl: 0, mo: m.label, txt: `Pacing ${ldSignedPct(m.revenueYoy)} ahead of same time last year, earning above market (RevPAR Index ${Math.round(m.revparIndex)}).` });
    } else if (m.revparIndex != null && m.revparIndex >= 105) {
      flags.push({ lvl: 1, mo: m.label, txt: `RevPAR Index ${Math.round(m.revparIndex)} — earning above the market average.` });
    } else if (m.occ != null && m.marketOcc != null && m.occ > m.marketOcc + 5) {
      flags.push({ lvl: 1, mo: m.label, txt: `Occupancy ${Math.round(m.occ)}% vs market ${Math.round(m.marketOcc)}% — ahead of comparable listings.` });
    } else if (m.revenueYoy != null && m.revenueYoy > 10) {
      flags.push({ lvl: 2, mo: m.label, txt: `Booking ${ldSignedPct(m.revenueYoy)} ahead of last year's pace.` });
    }
  });
  flags.sort((a, b) => a.lvl - b.lvl);
  const tone = [
    { label: 'Ahead', color: c.success, bg: c.successBg },
    { label: 'Strong', color: c.success, bg: c.successBg },
    { label: 'Pacing up', color: c.success, bg: c.successBg },
  ];
  const focus = `<div class="ld-card"><p class="ld-card-title">Highlights</p><p class="ld-card-desc">Forward months · where this listing is winning</p>${flags.length === 0
    ? `<p class="ld-empty">Highlights will appear here as upcoming months build momentum.</p>`
  ...
  return kpis + chart + focus + renderRecentReservations(reservations);
}
...
function renderListingDetail(client, listing, data, reservations) {
  const asOf = data.asOf;
  const panels = {
    overview: ldOverview(listing, asOf, reservations),
...
// Single template literal on purpose — same SWC folding bug as pricingBackHref.
function seoBackHref(client) {
  return `/api/dashboard/${encodeURIComponent(client.id)}?tab=seo&token=${encodeURIComponent(client.token || '')}`;
}
...
// Wrapped in a g.fn-row so the CSS-only hover spotlight (defs <style> below)
// can scale the stage and dim its siblings. geo.inSize shrinks the in-polygon
// value font on the narrow bottom stages so numbers never spill onto the
// light background (white-on-paper = invisible).
function seoFunnelRow(listing, key, geo) {
  ...
  const inSize = geo.inSize ? ` font-size="${geo.inSize}"` : '';
  return `
          <g class="fn-row">
          <polygon points="${geo.pMy}" fill="${geo.fillMy}"></polygon>
          <polygon points="${geo.pSim}" fill="${geo.fillSim}"></polygon>
          <text x="${geo.myX}" y="${geo.inY}" text-anchor="middle" class="fn-in"${inSize}>${myStr}${myWins ? SEO_STAR : ''}</text>
          <text x="${geo.simX}" y="${geo.inY}" text-anchor="middle" class="fn-in"${inSize}>${simStr}${myWins ? '' : SEO_STAR}</text>
...
function renderSeoFunnel(listing, seo) {
  ...
  const rows =
    ...
    seoFunnelRow(listing, 'booking_rate', {
      ...
      fillMy: '#3a6b5d', fillSim: '#a48274', myX: 332, simX: 428, inY: 375, nameY: 366, valY: 387, inSize: 12,
    }) +
    seoFunnelRow(listing, 'overall_conversion', {
      pMy: '290,404 378,404 378,470 298,470', pSim: '382,404 470,404 462,470 382,470',
      fillMy: '#45796a', fillSim: '#ad8e80', myX: 336, simX: 424, inY: 441, nameY: 432, valY: 453, inSize: 11,
    });
      ...
      <div class="funnel-card">
        ...
        <svg class="chart-svg" viewBox="0 0 760 478" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Booking funnel: your property vs similar listings">
          <defs><style>
            ...
            /* Hover spotlight — CSS-only enhancement; static render unaffected. */
            g.fn-row{ transition:opacity .2s ease; }
            svg:has(g.fn-row:hover) g.fn-row:not(:hover){ opacity:.45; }
            g.fn-row polygon{ transition:transform .2s ease; transform-box:fill-box; transform-origin:center; }
            g.fn-row:hover polygon{ transform:scale(1.04); }
          </style></defs>
          <g class="fn-row">
...
function renderDashboard(client, data, { tab = 'pricing', seo = null, listing = null, reservations = null } = {}) {
  const isSeo = tab === 'seo';
  ...
  const body = isSeo
    ...
    : pickedListing
    ? renderListingDetail(client, pickedListing, data, reservations)
    : data.source === 'report'

...
function mapMetricRow(m) {
  ...
  return {
    ...
    revenueStly,
    revenueYoy,
    revenueLy: num(m.rental_revenue_ly), // last year's final close (full month)
    adr: num(m.rental_adr),
    adrYoy: num(m.rental_adr_stly_yoy_pct),
    marketAdr: num(m.market_adr),
    marketAdrYoy: num(m.market_adr_stly_yoy_pct),
    revpar: num(m.rental_revpar),
    marketRevpar: num(m.market_revpar),
    revparIndex: num(m.revpar_index),
    occ: num(m.adjusted_occupancy_pct),
    occLy: num(m.adjusted_occupancy_ly_pct),
    marketOcc: num(m.market_occupancy_pct),
    mpi: num(m.market_penetration_index_pct),
    bookingWindow: num(m.median_booking_window),
    bookingWindowLy: num(m.median_booking_window_ly),
    marketBookingWindow: num(m.market_median_booking_window),
  ...
  const { data: metricRows, error: metricErr } = await sb
    ...
    .select(
      'listing_id, period, rental_revenue, rental_revenue_stly, rental_revenue_stly_yoy_pct, ' +
        'rental_revenue_ly, rental_adr, rental_adr_stly_yoy_pct, market_adr, market_adr_stly_yoy_pct, ' +
        'rental_revpar, market_revpar, revpar_index, ' +
        'adjusted_occupancy_pct, adjusted_occupancy_ly_pct, market_occupancy_pct, market_penetration_index_pct, ' +
        'median_booking_window, median_booking_window_ly, market_median_booking_window, ' +
